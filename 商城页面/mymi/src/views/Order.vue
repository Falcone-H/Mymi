      ...
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <li class="order-info">
            ...
            <div class="order-time">订单时间: {{dateFormat(item[0].order_time)}}</div>
          </li>
          <li class="header">
          ...
          <!-- 订单列表 -->
          <li class="product-list" v-for="(elem ,i) in item" :key="i">
            <div class="pro-img">
              <router-link :to="{ path: '/goods/details', query: {productID:elem.product_id} }">
                <img :src="$target + elem.product.product_picture" />
              </router-link>
            </div>
            <div class="pro-name">
              <router-link :to="{ path: '/goods/details', query: {productID:elem.product_id} }">
                {{elem.product.product_name}}</router-link>
            </div>
            <div class="pro-price">{{elem.product_price}}元</div>
            <div class="pro-num">{{elem.product_num}}</div>
            <div class="pro-total pro-total-in">{{elem.product_price*elem.product_num}}元</div>
          </li>
        </ul>
        <div class="order-bar">
