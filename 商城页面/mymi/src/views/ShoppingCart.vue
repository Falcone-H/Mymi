    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          <div class="pro-check">
            <el-checkbox v-model="item.check" @change="checkChange($event,index)" border></el-checkbox>
          </div>
          <div class="pro-img">
          ...
          <div class="pro-total pro-total-in">{{item.product_price*item.num}}元</div>
          <div class="pro-action" @click="deleteItem(item.product_id)">
...
export default {
  ...
  methods: {
    ...
    handleChange(currentValue, key, productID) {
      this.$axios
        .post("/api/user/updateShoppingCart", {
    ...
    deleteItem(productID) {
      this.$axios
        .post("/api/user/deleteFromShoppingCart", {
          user_id: this.$store.getters.getUser.id,
          product_id: productID,
        })
        .then((res) => {
